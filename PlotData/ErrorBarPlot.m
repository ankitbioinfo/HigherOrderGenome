...
a(1,:,:)=load('./HUVEC/Avg_COM_errorbar.dat');
...
for i=1:Row
	...
	if chro<7
        ...
        %         'MarkerFaceColor','b',...
        %         'MarkerSize',10); 
        k= plot(xax,kalhore(:,2),'bo','MarkerFaceColor','none','MarkerSize',10);
	...
	textsymbol=[yy+zz' yy-zz' kalhore(:,2)-0.02 kalhore(:,2)+0.02];
        ...
        set(gca,'FontSize',18)
        hl=legend([h,k],'Sim','Exp','Location','SouthEast','Orientation','horizontal');
	set(hl,'color','none'); set(hl, 'Box', 'off');
...
saveas(gcf, 'ErrorbarT_1_6_12', 'pdf')
